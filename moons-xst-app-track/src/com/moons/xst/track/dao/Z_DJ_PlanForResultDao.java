 ...
 */
public class Z_DJ_PlanForResultDao extends AbstractDao<Z_DJ_Plan, Void> {
	...
	public Z_DJ_PlanForResultDao(DaoConfig config) {
		super(config);
	}

	public Z_DJ_PlanForResultDao(DaoConfig config, DJResultDAOSession daoSession) {
	...
	public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
		...
		db.execSQL("CREATE TABLE " + constraint + "'Z_DJ_PLAN' (" + //
				...
				"'STANDARDVALUE_TX' TEXT," + // 21: StandardValue_TX
		...
		if (AppContext.getCurrLineType() == AppConst.LineType.CaseXJ.getLineType()) {
			...
			Z_DJ_PlanDao djplanDao = djdaoSession.getZ_DJ_PlanDao();
			List<Z_DJ_Plan> planList = djplanDao.loadAll();
			
			for (Z_DJ_Plan plan : planList) {
				ContentValues cv = new ContentValues();
	            cv.put("DJ_PLAN_ID", plan.getDJ_Plan_ID());
	            cv.put("PLANDESC_TX", plan.getPlanDesc_TX());
	            cv.put("SORTNO_NR", plan.getSortNo_NR());
	            cv.put("ESTSTANDARD_TX", plan.getESTStandard_TX());
	            cv.put("DATATYPE_CD", plan.getDataType_CD());
	            cv.put("ZHENDONG_TYPE", plan.getZhenDong_Type());
	            cv.put("ZHENDONG_PP", plan.getZhenDong_PP());
	            cv.put("CYCLE_ID", plan.getCycle_ID());
	            cv.put("CYCLEBASEDATE_DT", plan.getCycleBaseDate_DT());           
	            cv.put("IDPOS_ID", plan.getIDPos_ID());
	            cv.put("CHECKMETHOD", plan.getCheckMethod());
	            cv.put("ALARMTYPE_ID", plan.getAlarmType_ID());
	            cv.put("LOWERLIMIT1_TX", plan.getLowerLimit1_TX());
	            cv.put("LOWERLIMIT2_TX", plan.getLowerLimit2_TX());
	            cv.put("LOWERLIMIT3_TX", plan.getLowerLimit3_TX());
	            cv.put("LOWERLIMIT4_TX", plan.getLowerLimit4_TX());
	            cv.put("UPPERLIMIT1_TX", plan.getUpperLimit1_TX());
	            cv.put("UPPERLIMIT2_TX", plan.getUpperLimit2_TX());
	            cv.put("UPPERLIMIT3_TX", plan.getUpperLimit3_TX());
	            cv.put("UPPERLIMIT4_TX", plan.getUpperLimit4_TX());           
	            cv.put("METRICUNIT_TX", plan.getMetricUnit_TX());
	            cv.put("STANDARDVALUE_TX", plan.getStandardValue_TX());
	            cv.put("PARMLOWERLIMIT_TX", plan.getParmLowerLimit_TX());
	            cv.put("PARMUPPERLIMIT_TX", plan.getParmUpperLimit_TX());
	            cv.put("DATACODEGROUP_ID", plan.getDataCodeGroup_ID());
	            cv.put("MUSTCHECK_YN", plan.getMustCheck_YN());
	            cv.put("SPECCASE_YN", plan.getSpecCase_YN());
	            cv.put("SPECCASE_TX", plan.getSpecCase_TX());
	            cv.put("WENDU_FSL", plan.getWenDu_FSL());
	            cv.put("DISSTART_TM", plan.getDisStart_TM());
	            cv.put("DISEND_TM", plan.getDisEnd_TM());
	            cv.put("LKPOINT_ID", plan.getLKPoint_ID());
	            cv.put("SRPOINT_ID", plan.getSRPoint_ID());
	            cv.put("MOBJECTSTATE_TX", plan.getMObjectState_TX());
	            cv.put("DEFAULTRESULT_TX", plan.getDefaultResult_TX());
	            cv.put("MOBJECTBUGSPEC_ID", plan.getMobjectBugSpec_ID());
	            cv.put("SENSORRATIO_TX", plan.getSensorRatio_TX());
	            cv.put("LASTRESULT_TX", plan.getLastResult_TX());
	            cv.put("WARNINGVALUE_NR", plan.getWARNINGVALUE_NR());
	            cv.put("LASTCOMPLETE_DT", plan.getLastComplete_DT());           
	            cv.put("TRANSINFO_TX", plan.getTransInfo_TX());
	            cv.put("CONLEVEL_TX", plan.getConLevel_TX());
	            cv.put("PLANCYCLEIDSSTR_TX", plan.getPlanCycleIDsStr_TX());
	            cv.put("GROUPNUM_NR", plan.getGroupNum_NR());
	            cv.put("MAXFREQ_NR", plan.getMaxFreq_NR());
	            cv.put("FFTLINE_NR", plan.getFFTLine_NR());
	            cv.put("WINDOWTYPE_NR", plan.getWindowType_NR());
	            cv.put("AVERNUM_NR", plan.getAverNum_NR());            
	            cv.put("SPEED_YN", plan.getSpeed_YN());
	            cv.put("AVERAGETYPE_NR", plan.getAverageType_NR());
	            cv.put("OVERLAP_NR", plan.getOverlap_NR());           
