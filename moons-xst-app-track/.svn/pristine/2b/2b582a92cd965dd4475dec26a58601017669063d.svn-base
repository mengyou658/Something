...
import com.moons.xst.track.bean.GesturePoint;
import com.moons.xst.track.common.AppUtil;
import com.moons.xst.track.common.Constants;
...
public class GestureDrawline extends View {
	...
	private Hashtable<String, String[]> gestureUserInfo;
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
		...
		paint.setColor(Color.parseColor(getResources().getString(R.color.xstblue)));// 设置默认连线颜色
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
			Hashtable<String, String[]> hash, GestureCallBack callBack) {
		super(context);
		screenDispaly = AppUtil.getScreenDispaly(context);
		paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
		bitmap = Bitmap.createBitmap(screenDispaly[0], screenDispaly[0], Bitmap.Config.ARGB_8888); // 设置位图的宽高
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			if (isVerify) {
				...
				Set<String> keys = gestureUserInfo.keySet();
				for (String key : keys) {
					if (gestureUserInfo.get(key)[1].toString().equals(passWordSb.toString())) {
						account = key;
						password = gestureUserInfo.get(key)[0].toString();
						flag = true;
						break;
					}
				}
				if (flag) {
					// 代表用户绘制的密码手势与传入的密码相同
					callBack.checkedSuccess(account, password);
				} else {
					// 用户绘制的密码与传入的密码不同。
					callBack.checkedFail(getResources().getString(R.string.msg_geasture_login_fail));
				}
			} else {
	...
	private void drawErrorPathTip() {
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(Color.parseColor(getResources().getString(R.color.red)));// 设置默认线路颜色
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
	...
	public interface GestureCallBack {
		 ...
		 * 代表用户绘制的密码与传入的密码相同
		 */
		public abstract void checkedSuccess(String account, String password);

		/**
		 * 代表用户绘制的密码与传入的密码不相同
		 */
		public abstract void checkedFail(String emsg);
	}
