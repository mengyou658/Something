package com.camera.camera.view;
...
public class CameraView extends SurfaceView implements CameraOperation {
	...
	public boolean startRecord() {
		...
		//mMediaRecorder.setMaxFileSize(5*1024*1024); //设置视频最大大小
		mMediaRecorder.setMaxDuration(15*1000);//设置视频的最大时长，单位毫秒
		String path = FileOperateUtil.getFolderPath(getContext(),
				FileOperateUtil.TYPE_VIDEO, AppConst.XSTTempPicPath());
		...
		if (mRecordPath != null) {
			...
			if (bitmap != null) {
				String mThumbnailFolder = FileOperateUtil.getFolderPath(
						getContext(), FileOperateUtil.TYPE_THUMBNAIL,
						AppConst.XSTTempPicPath());
	...
	private void setCameraParameters() {
		try {
			...
			if (sizeList.size() > 0) {
				Size cameraSize = sizeList.get(sizeList.size() - 5);
				// 预览图片大小
			...
			if (sizeList.size() > 0) {
				...
				for (Size size : sizeList) {
					// 小于100W像素
					if (300 * 10000 < size.width * size.height
							&& size.width * size.height < 500 * 10000) {
						cameraSize = size;
						break;
					}
				}
	...
	private void updateCameraOrientation() {
		if (mCamera != null) {
			...
			if (!mIsFrontCamera)
			{
				...
				if (!AppConst.MS600YN())  //MS600前置摄像头不旋转
				{
					// 预览图片旋转90°
					mCamera.setDisplayOrientation(90);// 预览转90°
				}
