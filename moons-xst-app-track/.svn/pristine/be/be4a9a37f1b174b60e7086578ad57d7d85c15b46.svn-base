...
public class FileOperateUtil {
	...
	public static boolean deleteThumbFile(String thumbPath,Context context) {
		...
		flag = file.delete();
		String thumbname = getFileName(thumbPath);
		String result = thumbname.substring(0, 5);
		String sourcePath = null;
		//源文件路径
		if (result.equals("video")) {
			sourcePath=thumbPath.replace(context.getString(R.string.Thumbnail),
					context.getString(R.string.Video)).replace("jpg","3gp");
		}
		else {
			 sourcePath=thumbPath.replace(context.getString(R.string.Thumbnail),
	...
	public static boolean deleteSourceFile(String sourcePath,Context context) {
		...
		String result = sourcename.substring(0, 5);
		String thumbPath = null;
		if (result.equals("video")) {
			 thumbPath=sourcePath.replace(context.getString(R.string.Video),
						context.getString(R.string.Thumbnail)).replace("3gp", "jpg");
		}
		else {
			//缩略图文件路径
