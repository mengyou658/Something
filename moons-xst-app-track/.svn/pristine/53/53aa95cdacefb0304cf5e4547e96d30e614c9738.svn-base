...
import com.camera.album.view.MatrixImageView.OnMovingListener;
import com.camera.album.view.MatrixImageView.OnSingleTapListener;
import com.camera.imageloader.DisplayImageOptions;
import com.camera.imageloader.ImageLoader;
import com.camera.imageloader.displayer.MatrixBitmapDisplayer;
...
//					throw new RuntimeException("onPlayVideoListener is null");
		...
		public String deleteCurrentItem(int position) {
			String path=paths.get(position);
			if(path!=null) {
				FileOperateUtil.deleteThumbFile(path, getContext());//修改的
				paths.remove(path);
