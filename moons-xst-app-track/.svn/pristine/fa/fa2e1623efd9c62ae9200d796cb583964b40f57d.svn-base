...
public class CalendarUtil {
	...
	public static List<DateBean> getMonthDate(int year, int month) {
		...
		for (int i = 0; i < week; i++) {
			datas.add(initDateBean(lastYear, lastMonth, lastMonthDays - week
					+ 1 + i, 0));
		}
		...
		for (int i = 0; i < 7 * getMonthRows(year, month) - currentMonthDays
				- week; i++) {
	...
	private static DateBean initDateBean(int year, int month, int day, int type) {
		...
		if (type == 0) {
			dateBean.setSolarHoliday(SolarUtil.getSolarHoliday(year, month,
					day - 1));
		} else {
			dateBean.setSolarHoliday(SolarUtil
					.getSolarHoliday(year, month, day));
		}
		return dateBean;
	}
	...
	public static int getMonthRows(int year, int month) {
		int items = SolarUtil.getFirstWeekOfMonth(year, month - 1)
				+ SolarUtil.getMonthDays(year, month);
