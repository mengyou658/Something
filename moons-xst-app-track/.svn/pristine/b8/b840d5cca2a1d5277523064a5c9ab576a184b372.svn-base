...
public class WorkBillDownLoad {

	private static WorkBillDownLoad workBillDownLoad;
	...
	private static final int UPLOADING_UNABLE = 3;// 不能上传

	public static WorkBillDownLoad getWorkBillDownLoad(Context context,
			int index) {
		if (workBillDownLoad == null) {
			workBillDownLoad = new WorkBillDownLoad();
		}
		workBillDownLoad.mContext = context;
		workBillDownLoad.workBillType = AppConst.WorkBillType(index);
		workBillDownLoad.interceptFlag = false;
		return workBillDownLoad;
	}
	...
	private Handler uploading = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {// 上传成功
				...
				if (msg.arg1 == 1) {// 上传成功
					// 上传成功清空WorkBill文件夹
					String path = AppConst.WorkBillRecordPath();
					com.moons.xst.track.common.FileUtils
							.clearFileWithPath(path);
					UIHelper.ToastMessage(
							mContext,
							mContext.getString(R.string.cumm_cummdownload_uploadfinished));
				}
				encodeAndDownloadWorkBill();
			} else if (msg.what == 2) {// 不能上传
				// 关闭并释放进度条对话框
				if (mProDialog != null) {
					mProDialog.dismiss();
					mProDialog = null;
				}
				showFailDialog(msg.obj.toString(), UPLOADING_UNABLE);
			}
		}
	};

	// 上传工作票
	public void uploadingData() throws InterruptedException {
		if (mProDialog == null) {
			mProDialog = ProgressDialog.show(mContext, null,
					mContext.getString(R.string.twobill_work_uploading_hint),
					true, true);
		...
		if (!WorkBillHelper.GetIntance().WorkBillCanUpload(mContext)) {
			Message msg = Message.obtain();
			msg.what = 2;
			msg.obj = mContext
					.getString(R.string.workDownLoad_unable_uploading);
		...
		new Thread() {
			public void run() {
				try {
					...
					String json = WorkResultHelper.GetIntance().getWorkJson(
							mContext);
					...
					ReturnResultInfo detail = WebserviceFactory.WorkUploading(
							mContext, json);
					_msg = Message.obtain();
					if (detail.getResult_YN()) {
						...
						OperatingConfigHelper.getInstance().i(AppConst.CONNECTIONTYPE_WIFI +
								AppConst.COMMTYPE_WORKBILL_UPLOADING,
	 ...
	 * 标准化下载工作票
	 */
	public void encodeAndDownloadWorkBill() {
					...
					try {
						...
						if (!returnInfo.getResult_YN()) {
							...
							ReturnResultInfo detail = WebserviceFactory
									.encodeWorkBillInfo(mContext, null, workBillType);
	...
	private void showFailDialog(String msg, int type) {
		try {
			...
			FailDialog = new com.moons.xst.track.widget.AlertDialog(mContext)
					...
					.setView(view).setMsg(message + msg)
	...
	public void showDownloadDialog() {
		...
		Dialog = new com.moons.xst.track.widget.AlertDialog(mContext)
				.builder()
				.setTitle(mContext.getString(R.string.workDownLoad_download_ticket))
				.setView(view)
	...
	private Runnable mdownDBRunnable = new Runnable() {
		...
		public void run() {
			try {
				...
				String dbName = "WorkBill.sdf";
				String tmpDB = "WorkBill.tmp";
				// 判断是否挂载了SD卡
				...
				do {
					...
					if (numread <= 0) {
						...
						if (tmpFile.renameTo(dbFile)) {
							...
							// 插入操作日志
							OperatingConfigHelper.getInstance().i(AppConst.CONNECTIONTYPE_WIFI 
									+ AppConst.COMMTYPE_WORKBILL_DOWNLOAD,
									AppConst.LOGSTATUS_NORMAL,
									"");
						}
						break;
